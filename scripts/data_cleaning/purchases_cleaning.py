...
def clean_purchases_fact(headers_file, lines_file, output_fact_file, output_date_file):
    """
    headers_file: purchase headers Excel
    lines_file: purchase lines Excel
    output_fact_file: cleaned purchases fact JSON
    ...
    # ---------------------
    if df["total_line_cost"].isnull().any():
        df["total_line_cost"] = df["unit_cost_price"] * df["quantity_purchased"]

    if "total_cost" in df.columns and df["total_cost"].isnull().all():
        df["total_cost"] = df.groupby("purchase_id")["total_line_cost"].transform("sum")
